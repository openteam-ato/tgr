...
function draw_map_municipal_readiness(){
  ...
  $(map_municipal_readiness_regions).each(function (index, region) {
    ...
    paper_item
      ...
      .click(function() {
        var object = this;
        if ($('.map_wrapper').hasClass('municipal_readiness')) {
          municipal_readiness_manipulate(object.stored_attr.name + '_item');
          return false;
        };
...
function municipal_readiness_manipulate(identifier) {
  var block = $('.' + identifier + '_links');
  if (block.is(':visible')) {
    return false;
  } else {
    var bottom_position = $('.municipal_readiness_links').position().top + $(block).outerHeight(true),
...
function check_hash_exist(paper) {
  ...
  var link = $('.municipal_readiness .map_desc ' + hash + '_item a');
  link.addClass('hover').click();
  paper.getById(link.attr('id')).attr({'fill': '#8cc88c', 'stroke':'#378437'});
};

function init_map_municipal_readiness() {
  var paper = draw_map_municipal_readiness();
  $('.map_desc li a').hover(
    function() {
    ...
    if ($('.map_wrapper').hasClass('municipal_readiness')) {
      municipal_readiness_manipulate($(this).closest('li').attr('id'));
      return false;
    };
  });
